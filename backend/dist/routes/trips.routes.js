...
const requiredParam = (value) => {
    if (!value)
        throw new AppError("Required route parameter is missing", 400, "MISSING_PARAM");
    return value;
};
...
tripsRouter.get("/", validate(z.object({
    ...
    res.json(await listTrips(req.user.id, req.query));
}));
...
tripsRouter.post("/:tripId/cover", upload.single("cover"), validate(z.object({ params: z.object({ tripId: z.string().uuid() }) })), asyncHandler(async (req, res) => {
    if (!req.file)
        throw new AppError("Cover image is required", 400, "COVER_REQUIRED");
