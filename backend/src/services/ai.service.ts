...
export async function generateItinerary(userId: string, tripId: string, input: { prompt: string; days?: number; interests?: string[] }) {
...
`);
  ...
  const plan = (ai ?? fallback) as typeof fallback;
...
export async function suggestPacking(input: { destination?: string; days?: number; season?: string; activities?: string[] }) {
  const ai = await askClaude(`
...
export async function recommendDestinations(input: { interests: string[]; budget?: number; region?: string }) {
